...
if opt.dataset == 'GTSRB':
    ...
    traindataloader = TrainDataloaderCIFAR10
    valdataloader = TestDataloaderCIFAR10
...
Target_labels = torch.stack([i*torch.ones(1) for i in range(opt.num_class)]).expand(
    opt.num_class, opt.batch_size).permute(1, 0).reshape(-1, 1).squeeze().to(dtype=torch.long)
