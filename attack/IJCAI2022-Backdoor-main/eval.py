...
Target_labels = 0*torch.ones(1,1).squeeze().to(dtype=torch.long).unsqueeze(0)
if opt.dataset == 'GTSRB':
    checkpoiont = torch.load('./well_trained/{}.pth'.format(opt.dataset, opt.model_type), map_location=torch.device('cpu'))
elif opt.dataset == 'CelebA':
    checkpoiont = torch.load('./well_trained/{}.pth'.format(opt.dataset, opt.model_type), map_location=torch.device('cpu'))
Classifer.load_state_dict(checkpoiont['netC'])
TriggerNet.load_state_dict(checkpoiont['netP'])
Classifer = Classifer
TriggerNet = TriggerNet
...
if __name__=='__main__':
    for ii in range(opt.num_class):
        Target_labels = ii*torch.ones(opt.batch_size).to(dtype=torch.long)
        with torch.no_grad():
